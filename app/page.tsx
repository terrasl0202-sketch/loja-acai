...
import { Minus, Plus, ShoppingCart, Send, MapPin, User, CreditCard, MessageSquare } from "lucide-react"
...
export default function Home() {
  ...
  const handleSubmit = () => {
    ...
    const orderItems = products
      ...
      .map((p) => `• ${quantities[p.id]}x ${p.name} - ${formatCurrency(p.price * quantities[p.id])}`)
      .join("\n")
...
${formData.observacao ? `📝 *Observação:* ${formData.observacao}` : ""}

Obrigado pela preferência! 💜`

    const whatsappUrl = `https://wa.me/5511918505799?text=${encodeURIComponent(message)}`
  ...
  return (
    <main className="min-h-screen bg-background pb-24">
      ...
      <div className="max-w-lg mx-auto px-4">
        ...
        {/* Order Form */}
        <section className="mt-8 bg-card rounded-2xl p-4 border border-border shadow-lg shadow-primary/5">
