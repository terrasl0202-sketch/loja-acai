...
// Funcao exportada para uso em outros modulos
export async function getCustomerVipStatus(customerId: number): Promise<CustomerVipStatus | null> {
  ...
  // Calcular total gasto pelo cliente (apenas pedidos validos para faturamento)
  ...
  let currentLevel = levels[0] // Bronze como padrao
...
// GET - Buscar status VIP do cliente
export async function GET(request: NextRequest) {
  try {
    ...
    // Se passou phone, buscar customer_id
    ...
    const status = await getCustomerVipStatus(customerIdNum)
