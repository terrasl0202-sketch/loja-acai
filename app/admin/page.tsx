...
*Link do Mapa:* ${mapsLink}`
  ...
  // 7. ARQUIVADOS - filtrar com busca especifica
  const ordersArchived = orders.filter(o => {
    if (!o.archived) return false
    
    // Aplicar filtro de busca especifico para arquivados
    if (archivedSearchQuery.trim()) {
      ...
      return matchId || matchName || matchPhone || matchPayment
    }
    return true
  })
  ...
  // Arquivar pedido individual
  const archiveOrder = async (orderId: string) => {
    try {
      const res = await fetch("/api/orders", {
        method: "PATCH",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({ password: sessionPassword, orderId, archived: true }),
      })
      const data = await res.json()
      if (data.success) {
        setOrders(prev => prev.map(o => o.id === orderId ? { ...o, archived: true } : o))
  ...
  // Restaurar pedido arquivado
  const restoreOrder = async (orderId: string) => {
    try {
      const res = await fetch("/api/orders", {
        method: "PATCH",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({ password: sessionPassword, orderId, archived: false }),
      })
      const data = await res.json()
      if (data.success) {
  ...
  // Executar arquivamento automatico baseado na configuracao
  const runAutoArchive = () => {
    ...
    orders.forEach(order => {
      ...
      // So arquivar finalizados ou cancelados
      if (order.status !== "completed" && order.status !== "cancelled") return
      
      const orderDate = new Date(order.createdAt).getTime()
      if (now - orderDate >= daysInMs) {
        archiveOrder(order.id)
      }
    })
  }

  // Executar arquivamento automatico quando config ou orders mudam
  useEffect(() => {
          ...
          <div className="grid lg:grid-cols-[280px_1fr] gap-6">
            ...
            <div className="bg-card rounded-2xl p-6 border border-border min-h-[600px]">
              ...
              {activeTab === "hours" && (
                <div className="space-y-6">
                  ...
                  <div className="space-y-4">
                    ...
                    <div className="pt-4 border-t border-border">
                      <label className="text-sm text-muted-foreground">Arquivamento automatico de pedidos</label>
                      <select
                        ...
                        className="w-full mt-1 px-4 py-3 bg-input border border-border rounded-xl text-foreground focus:outline-none focus:ring-2 focus:ring-primary"
                      >
                        <option value={0}>Nunca arquivar automaticamente</option>
                        <option value={7}>Apos 7 dias</option>
                        <option value={15}>Apos 15 dias</option>
                        <option value={30}>Apos 30 dias</option>
                      </select>
                      <p className="text-xs text-muted-foreground mt-1">Pedidos finalizados ou cancelados serao arquivados automaticamente apos o periodo selecionado. Pedidos arquivados continuam aparecendo nos relatorios.</p>
                    </div>
                  </div>
                </div>
              )}
                    ...
                    {ordersCompleted.map((order) => (
                      <div key={order.id} className="p-4 rounded-xl border border-border bg-secondary/30">
                        ...
                        <div className="flex flex-wrap gap-2 pt-3 border-t border-border">
                          ...
                          <button onClick={() => archiveOrder(order.id)} className="px-4 py-2 text-sm font-medium rounded-lg bg-slate-500/20 text-slate-400 hover:bg-slate-500/30 transition-all flex items-center gap-2">
              ...
              {activeTab === "orders-archived" && (
                <div className="space-y-6">
                  <div className="flex items-center justify-between">
                    <h2 className="text-xl font-bold text-foreground">Pedidos Arquivados ({ordersArchived.length})</h2>
                    <button onClick={loadOrders} className="flex items-center gap-2 px-4 py-2 bg-secondary text-foreground font-medium rounded-xl transition-all hover:bg-secondary/80">
                      <Loader2 className="w-4 h-4" /> Atualizar
                    </button>
                  </div>

                  {/* Busca de pedidos arquivados */}
                  <div className="flex gap-2">
                    <input
                      type="text"
                      placeholder="Buscar por numero, cliente, telefone ou pagamento..."
                      ...
                      className="flex-1 px-4 py-2 bg-secondary border border-border rounded-xl text-foreground placeholder:text-muted-foreground focus:outline-none focus:ring-2 focus:ring-primary/50"
                    />
                    <button
                      onClick={() => setArchivedSearchQuery(archivedSearchInput)}
                      className="flex items-center gap-2 px-4 py-2 bg-primary text-primary-foreground font-medium rounded-xl hover:bg-primary/90 transition-all"
                    >
