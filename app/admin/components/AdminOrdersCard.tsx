...
type TabType = "dashboard" | "store" | "products" | "categories" | "banner" | "hours" | "payment" | "whatsapp" | "delivery" | "coupons" | "entregadores" | "customization" | "backup" | "premium" | "orders-pending" | "orders-paid" | "orders-preparing" | "orders-delivering" | "orders-completed" | "orders-cancelled" | "orders-abandoned" | "orders-archived"
...
interface AdminOrdersCardProps {
  ...
  onSendToEntregador?: (order: Order) => void
}
      ...
      <div className="p-4 flex items-center justify-between border-b border-white/5">
        <div className="flex items-center gap-3">
          ...
          <div>
            ...
            <p className="text-[10px] text-gray-400">Clique para expandir</p>
          </div>
        </div>
        {onRefresh && (
        ...
        <div className="flex gap-2">
          <div className="relative flex-1">
            ...
            <input
              ...
              className="w-full pl-10 pr-4 py-2.5 text-sm bg-[#1a1a2e] border border-white/5 rounded-xl text-white placeholder:text-gray-400 focus:outline-none focus:ring-1 focus:ring-purple-500/50"
            />
          </div>
            ...
            <div key={tab.id} className="mb-1">
              ...
              {isExpanded && (
                <div className="mt-2 space-y-3 max-h-[600px] overflow-y-auto p-2">
                  {orders.length === 0 ? (
                    <p className="text-xs text-center text-gray-400 py-8">Nenhum pedido nesta categoria</p>
                  ) : (
                    orders.map((order) => (
                      <div key={order.id} className="p-4 bg-[#1a1a2e] rounded-xl border border-white/10">
                        ...
                        {order.address && (
                          <div className="mb-3">
                            ...
                            {order.neighborhood && <p className="text-[10px] text-gray-400">Bairro: {order.neighborhood}</p>}
                          </div>
                        )}
                            ...
                            <>
                              {isDelivery(order) ? (
                                ...
                                <div className="flex flex-col gap-2 w-full">
                                  {/* Mostrar entregador selecionado */}
                                  {order.entregadorNome && (
                                    <div className="p-2 bg-purple-500/10 border border-purple-500/30 rounded-lg">
                                      <p className="text-xs text-purple-400 font-medium">Entregador selecionado:</p>
                                      <p className="text-sm text-white font-bold">{order.entregadorNome}</p>
                                      <p className="text-xs text-gray-400">{order.entregadorWhatsapp}</p>
                                    </div>
                                  )}
                                  
                                  <div className="flex flex-wrap gap-2">
                                    {/* Botao Selecionar/Alterar Entregador */}
                                    <div className="relative">
                                      <button 
                                        onClick={() => setShowEntregadorList(showEntregadorList === order.id ? null : order.id)} 
                                        className={`flex items-center gap-1 px-3 py-2 text-xs rounded-lg ${order.entregadorNome ? 'bg-purple-600/30 text-purple-300 hover:bg-purple-600/50' : 'bg-purple-600 text-white hover:bg-purple-500'}`}
                                      >
                                        <Users className="w-3 h-3" /> {order.entregadorNome ? 'Alterar Entregador' : 'Selecionar Entregador'}
                                      </button>
                                        ...
                                        <div className="absolute top-full left-0 mt-1 w-64 bg-[#1a1a2e] border border-white/10 rounded-xl shadow-xl z-50 overflow-hidden">
                                          ...
                                          <div className="max-h-48 overflow-y-auto">
                                            {entregadores.length === 0 ? (
                                              <p className="p-3 text-xs text-gray-400 text-center">Nenhum entregador cadastrado</p>
                                            ) : (
                                              entregadores.filter(e => e.status === "ativo").map((entregador) => (
                                                ...
                                                >
                                                  ...
                                                  {(entregador.horarioInicio || entregador.horarioFim) && (
                                                    <p className="text-xs text-gray-400 mt-1">{entregador.horarioInicio} - {entregador.horarioFim}</p>
                                                  )}
                                                </button>
                                              ))
                                            )}
                                    ...
                                    {/* Botao Enviar para Entregador (WhatsApp) - so aparece quando tem entregador */}
                                    {order.entregadorNome && order.entregadorWhatsapp && onSendToEntregador && (
                                      <button 
                                        onClick={() => onSendToEntregador(order)} 
                                        className="flex items-center gap-1 px-3 py-2 text-xs bg-green-600 text-white rounded-lg hover:bg-green-500"
                                      >
                                        <MessageCircle className="w-3 h-3" /> Enviar para Entregador
                                      </button>
                                    )}
                                  </div>
                                </div>
